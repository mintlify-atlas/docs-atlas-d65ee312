...
<ParamField path="--out" type="string">
  Write the key to the specified file path. If omitted, the key is written to stdout.
</ParamField>
...
Generate a key and write it directly to a file:

```bash
pki --gen --type rsa --size 4096 --outform pem --out strongswanKey.pem
```
